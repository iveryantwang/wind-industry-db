...
## 🟢 2026-09-12 更新：融资关闭完成，项目进入建设期（`status` 核准 → 在建，`cod_year` 补为 2028）

**新事实：2026-09-11 Skyborn 宣布 Gennaker 达成融资关闭。以上全部既有内容不删不改，本段为增量。**

| 项 | 本次新增/确认 |
|---|---|
| **融资关闭日** | **2026-09-11** |
| 债务 | **EUR 21 亿无追索**，16 家商业银行 + **欧洲投资银行（EIB）** |
| 股本 | Skyborn + SWM（25%）补足 |
| 总投资 | **约 EUR 30 亿**（与本库既录的 `capex` 口径一致，无需修正） |
| PPA | Amazon **600MW** + Uniper **100MW** = 700MW，约 **72%** 容量覆盖 |
| 整机 | **Siemens Gamesa 63 台 SG 14-236**（与本库 2026-08-11 已查实的失单结论完全一致，再次交叉验证） |
| 单桩 | EEW |
| 过渡段 | 大金重工（与本库 2026-08-05 记录一致） |
| 基础运输安装 | Seaway7（与本库既录一致） |
| **风机吊装** | **Fred. Olsen Windcarrier** —— 本库此前未记录 |
| **海上安装窗口** | **2027 年年中 — 2028 年底** —— 本库此前未记录 |
| **全面投运** | **2028 年 12 月** —— 本库 `cod_year` 由**空**补为 **2028** |
| 来源 | <https://www.renews.biz/offshore-wind/skyborn-hits-e3bn-financial-close-on-gennaker/> |

### 字段变更说明

- `status`：**核准 → 在建**。融资关闭 + 过渡段已制造 + 安装窗口明确，项目已实质进入建设期。
- `cod_year`：**空 → 2028**。
- `opportunity`：**维持 `已失单` 不变**，2026-08-11 的结论与全部机会分析原文保留。

### 这条新信息对方法论的补充：把完整时间轴钉死

本条此前的两次复盘（08-05、08-11）已经得出「竞争在 MSA 阶段就已结束」的结论。融资关闭给出了这条时间轴的最后一个节点，现在可以完整排列：

| 环节 | 时点 | 距整机 TSA 公开 |
|---|---|---|
| **Skyborn–SGRE 框架供货协议 MSA** | **2024-06** | −13 个月 |
| **整机供货协议 TSA 公开** | **2025-07-18** | 0 |
| Amazon / Uniper PPA | 2026 上半年 | +约 11 个月 |
| SWM 入股 25% | 2026-06 | +11 个月 |
| 过渡段开工制造 | 2026-07/08 | +12–13 个月 |
| **融资关闭** | **2026-09-11** | **+14 个月** |
| 海上安装开始 | 2027 年年中 | +23 个月 |
| 全面投运 | 2028-12 | +41 个月 |

**可直接用于其他项目的判据：对欧洲成熟开发商的海上项目，整机定标普遍发生在融资关闭前约 12–18 个月、PPA 签署之前。** 因此在本库的其他条目上：

- 看到「PPA 签署」→ **不要**判定整机窗口开启，此时多半已定
- 看到「融资关闭」→ 整机几乎必然已定，应直接转入反查而非商务接触
- 真正的窗口在**框架供货协议（MSA）的谈判与换约期**，早于所有公开项目节点

详见 [[Gennaker976.5MW融资关闭与21亿欧元无追索债务包]]。

---

## 🟢 2026-09-22 更新（信息日 2026-09-21）：融资关闭后 10 天施工分包全面铺开

**以上全部既有内容不删不改，本段为增量。字段不变：`oem` 维持 `Siemens Gamesa`，`opportunity` 维持 `已失单`，`status` 维持 `在建`。**

| 项 | 内容 |
|---|---|
| **升压站上部组块装船** | 两座 OSS（**Darß**）在荷兰弗利辛恩完成装船待起运（2026-09-21） |
| **海事担保检验（MWS）** | **ABL** 获建设期 MWS 与海事保证服务合同（2026-09-21） |
| 场址距离 | 距 Fischland-Darß-Zingst 半岛以北约 **15 公里**（本库此前未记录） |
| 海上施工 / 投运 | 2027 年起 / 2028 年 —— 与本库既录一致 |
| 来源 | <https://www.offshorewind.biz/2026/09/21/abl-to-support-marine-operations-at-gennaker-offshore-wind-farm> · <https://www.offshorewind.biz/2026/09/21/gennaker-offshore-substation-topsides-loaded-out-ahead-of-sail-away> |

### ⚠️ 单机容量出现第三个口径，三者并列保留

| 来源 | 口径 |
|---|---|
| 本库推算（976.5 ÷ 63） | **15.5MW** |
| SGRE 官方机型标称（2025-07-18） | SG 14-236 DD，**额定最高 15MW** |
| **offshoreWIND.biz（2026-09-21）** | **最高 15.5MW** |

**本次报道与本库推算值一致。按「来源冲突并列写入、不取平均」原则，`unit_mw` 字段维持 15.5，正文并列记录 SGRE 官方的 15MW 口径。**

### 对既有方法论判据的加强

本条把 Gennaker 时间轴补到施工阶段：**融资关闭（2026-09-11）→ 10 天后升压站组块装船 + MWS 合同授出（2026-09-21）。**

**这说明融资关闭时整套施工分包已经就位，不是关闭之后才开始招。** 本库此前得出的判据「看到融资关闭 → 整机几乎必然已定，应直接转入反查而非商务接触」因此可以再收紧一句：

> **融资关闭不是窗口关闭的时点，是窗口关闭很久之后的公开确认。真正的关闭发生在框架供货协议（MSA）签署时，往往早于融资关闭 24 个月以上。**

Gennaker 实测：MSA（2024-06）→ 融资关闭（2026-09-11），**间隔 27 个月。**

---

## 关联
- [[2026-09-21-Gennaker两座升压站上部组块装船并授出海事担保检验合同]]
